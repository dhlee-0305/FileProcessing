...
public class ThreadPoolFramework{
	...
	/**
	 * <p>파일로 부터 데이타를 읽어와 작업 스레드에 작업을 할당한다.</p>
	 */
		...
		while(true){
			...
			synchronized(THREAD_POOL){
				for(ThreadRunner threadRunner : THREAD_POOL){
					//System.out.println("check pushJob for jobIndex : "+jobIndex+", Object : "+obj.toString());
					if(threadRunner.isReady()){
						...
						try{
							...
							System.out.println("check pushJob for threadIndex:"+threadRunner.getThreadIndex()+", jobIndex: "+jobIndex+", Object: "+obj.toString());
							e.printStackTrace();
						}
