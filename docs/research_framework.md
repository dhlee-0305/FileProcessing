...
| `ThreadJob` | `src/framework/ThreadJob.java` | 작업 인덱스, 처리 대상 데이터, 워커 종료 신호 여부를 담는 작업 단위 객체다. 실행 직전 워커 스레드 인덱스가 설정된다. |
...
main thread
  ...
  └─ new ThreadPoolFramework(worker, repository, N)
       ...
       │    ├─ worker thread 0 take()
       │    ├─ worker thread 1 take()
       │    └─ worker thread N-1 take()
       └─ scheduler thread
            ├─ repository.getData()
            ├─ ThreadJob 생성
            ├─ shared queue에 put()
            └─ EOF 이후 worker 수만큼 shutdown job put()
```
...
try
  ...
  worker 수만큼 shutdown job을 shared queue에 put()
  checkEOF = true
```
...
6. EOF 또는 예외 후 `finally`에서 워커 수만큼 shutdown job을 넣고 `checkEOF = true`로 설정한다.

`checkEOF`와 `schedulerException`은 `volatile`로 선언되어 있어 메인 스레드와 스케줄러 스레드 사이에서 값 변경이 관찰되도록 의도되어 있다.
...
이전처럼 워커 리스트를 순회하거나 `ThreadRunner`의 ready 상태를 확인하지 않는다. 모든 워커는 같은 큐에서 `take()`를 호출하며, 먼저 작업을 소비할 수 있는 워커가 다음 `ThreadJob`을 가져간다.

현재 큐 용량은 `maxThreadCount * 2`다. 입력 읽기 속도가 처리 속도보다 빠를 때 스케줄러가 작업을 무제한 적재하지 않도록 제한하고, 큐가 가득 차면 `put()`에서 대기한다. 이 구조는 유휴 워커를 polling하는 방식보다 지연이 작고, 큐가 스케줄러와 워커 사이의 동기화 지점이 된다.
...
```text
while (true)
  threadJob = jobQueue.take()
  threadJob이 shutdown이면 종료
...
1. 워커는 공유 `ThreadData` 큐에서 `take()`를 호출한다.
2. 큐가 비어 있으면 `take()`에서 블로킹되어 대기한다.
3. 일반 작업을 받으면 자기 `threadIndex`를 `ThreadJob`에 설정한다.
4. 주입받은 `ThreadWorker` 구현체의 `serve(threadJob)`을 호출한다.
5. shutdown job을 받으면 `run()`을 종료한다.
...
```text
finishingJob()
  checkEOF가 true가 될 때까지 schedulerThread.join()
  waitAllThread()
...
스케줄러는 EOF를 만나거나 예외가 발생하면 `finally`에서 워커 수만큼 shutdown job을 큐에 넣는다. 각 워커는 큐에서 shutdown job을 하나씩 가져가 정상적으로 종료한다. `waitAllThread()`는 `THREAD_POOL`에 보관된 모든 `ThreadRunner`를 꺼내 `join()`으로 실제 스레드 종료를 기다린다.

이 방식은 특정 워커의 ready 상태나 리스트 순서에 의존하지 않는다. 작업이 남아 있으면 shutdown job은 일반 작업 뒤에 들어가므로, 워커들은 이미 큐에 들어간 작업을 처리한 뒤 종료 신호를 받는다.
...
`SimpleFileParse`는 먼저 `getCheckEOF()`가 `true`가 될 때까지 메인 스레드에서 100ms 단위로 기다린 뒤 `finishingJob()`을 호출한다. EOF 감지는 스케줄러가 shutdown job 삽입까지 마친 뒤 설정되며, 실제 워커 완료와 종료 정리는 `finishingJob()`에서 수행한다.
...
| 워커 스레드 | `maxThreadCount` | 공유 큐에서 작업을 꺼내 `ThreadWorker.serve()` 실행 | 큐가 비어 있을 때 `LinkedBlockingQueue.take()` |
...
| 대상 | 방식 | 목적 |
| --- | --- | --- |
| `checkEOF` | `volatile` | 스케줄러에서 EOF 및 shutdown job 삽입 완료를 설정하고 메인 스레드가 관찰한다. |
...
| `ThreadJob.shutdown` | 작업 객체 플래그 | 일반 작업과 워커 종료 신호를 구분한다. |
...
스케줄러 예외는 `ThreadPoolFramework.threadScheduler()`에서 잡아 `schedulerException`에 저장한다. 이후 `finally`에서 shutdown job을 넣어 워커가 종료될 수 있게 한 뒤 `checkEOF`를 설정한다. `finishingJob()`은 스케줄러와 워커 종료를 기다린 뒤 이 예외를 다시 던진다.
...
6. shutdown job은 일반 작업 뒤에 삽입된다. 스케줄러 예외가 발생한 경우에도 이미 큐에 들어간 작업은 처리된 뒤 종료된다. 즉 예외 발생 즉시 모든 작업을 중단하는 fail-fast 구조는 아니다.
...
즉, 병렬화되는 구간은 `DataRepository.getData()`가 아니라 `ThreadWorker.serve()` 실행 구간이다. 파일 읽기는 단일 스케줄러에서 순차 수행되고, 읽힌 각 레코드의 후속 처리가 여러 워커 스레드에서 동시에 실행되는 구조다. 종료는 EOF 이후 워커 수만큼 shutdown job을 큐에 넣고 `join()`으로 실제 스레드 종료를 기다리는 방식으로 처리된다.
